...
async function _fetchDir(diskDir, user) {
    ...
    const url = `api.php?dir=${encodeURIComponent(diskDir)}&action=detail&u=${encodeURIComponent(user)}&t=dir`;
...
async function _fetchFilePage(diskDir, user, offset = 0, limit = FILE_PAGE) {
    const url = `api.php?dir=${encodeURIComponent(diskDir)}&action=detail&u=${encodeURIComponent(user)}&t=file&p=${offset}&n=${limit}`;
...
async function _fetchUserList(diskDir) {
    const url = `api.php?dir=${encodeURIComponent(diskDir)}&action=detail`;
    const res = await fetch(url);
    if (!res.ok) return [];
    const json = await res.json();
