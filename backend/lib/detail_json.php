...
function api_detail_page_index($ctx) {
    if (!isset($ctx['manifest']['page_index']) || empty($ctx['manifest']['page_index'])) return false;
    $rel = str_replace(array('/', '\\'), DIRECTORY_SEPARATOR, $ctx['manifest']['page_index']);
