...
function api_detail_simple_json_load($path) {
...
function api_detail_simple_manifest_path($disk_path) {
    return $disk_path . DIRECTORY_SEPARATOR . 'detail_users' . DIRECTORY_SEPARATOR . 'data_detail.json';
}

function api_detail_simple_find_ctx($disk_path, $who) {
    $root_path = api_detail_simple_manifest_path($disk_path);
    if (!is_file($root_path)) return false;
    $root = api_detail_simple_json_load($root_path);
    ...
    $manifest = api_detail_simple_json_load($manifest_path);
...
function api_detail_simple_ext($path) {
    return strtolower((string)pathinfo((string)$path, PATHINFO_EXTENSION));
}

function api_detail_simple_path_dict($detail_dir) {
    static $cache = array();
...
function api_detail_simple_filters($is_file) {
    return array(
...
function api_detail_simple_match($row, $is_file, $filters) {
    ...
    if ($is_file && $filters['ext'] !== '') {
        ...
        $xt = isset($row['xt']) ? strtolower((string)$row['xt']) : api_detail_simple_ext($path);
...
function api_detail_simple_collect_rows($ctx, $kind, $offset, $limit, $filters) {
    $is_file = ($kind === 'files');
...
function api_detail_simple_empty_dir($who, $offset, $limit) {
...
function api_detail_simple_empty_file($who, $offset, $limit) {
...
function api_detail_simple_date($ctx) {
...
function api_detail_simple_total_used($ctx) {
...
function api_handle_dirs($disk_path) {
    ...
    if (!$ctx) {
        ...
        b64_success(array('dir' => api_detail_simple_empty_dir($who, $offset, $limit)));
    }

    $result = api_detail_simple_collect_rows($ctx, 'dirs', $offset, $limit, api_detail_simple_filters(false));
    $payload = array(
        'date' => api_detail_simple_date($ctx),
        'user' => $who,
        'total_dirs' => (int)$result['total'],
        'total_dirs_full' => isset($ctx['entry']['dirs']) ? (int)$ctx['entry']['dirs'] : (int)$result['total'],
        'total_used' => api_detail_simple_total_used($ctx),
        'offset' => $offset,
        'limit' => $limit,
...
function api_handle_files($disk_path) {
    ...
    if (!$ctx) {
        ...
        b64_success(array('file' => api_detail_simple_empty_file($who, $offset, $limit)));
    }

    $result = api_detail_simple_collect_rows($ctx, 'files', $offset, $limit, api_detail_simple_filters(true));
    $payload = array(
        'date' => api_detail_simple_date($ctx),
        'user' => $who,
...
function api_handle_detail_simple($disk_path) {
    $who = sanitize_name(get_b64_param('user', ''));
    ...
    if (!$ctx) {
        b64_success(array(
            'dir' => api_detail_simple_empty_dir($who, $dir_offset, $limit),
            'file' => api_detail_simple_empty_file($who, $file_offset, $limit),
        ));
    }

    $dir = api_detail_simple_empty_dir($who, $dir_offset, $limit);
    if ($node_type !== 'file') {
        $dr = api_detail_simple_collect_rows($ctx, 'dirs', $dir_offset, $limit, api_detail_simple_filters(false));
        $dir = array(
            'date' => api_detail_simple_date($ctx),
            'user' => $who,
    ...
    $file = api_detail_simple_empty_file($who, $file_offset, $limit);
    if ($node_type !== 'dir') {
        $fr = api_detail_simple_collect_rows($ctx, 'files', $file_offset, $limit, api_detail_simple_filters(true));
        $file = array(
            'date' => api_detail_simple_date($ctx),
            'user' => $who,
