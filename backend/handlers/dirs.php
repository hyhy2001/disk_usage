...
function api_dirs_normalize_row($obj) {
    return api_detail_normalize_dir($obj);
}
...
function api_handle_dirs_csv($disk_path) {
    ...
    $filtered = api_detail_filter_sort_slice($rows, 'dir', 0, PHP_INT_MAX, $filter_q, '', $filter_min, $filter_max);
...
function api_handle_dirs($disk_path) {
    ...
    api_send_etag_cache($manifest_path, array('dirs', $who, $offset, $limit, $filter_q, $filter_min, $filter_max, param('count_only', '0'), @filemtime($manifest_path)));

    $dir = api_detail_get_dir_payload($disk_path, $who, $offset, $limit, $filter_q, $filter_min, $filter_max);
    if (param('count_only', '0') === '1') b64_success(array('dir_count' => isset($dir['total_dirs']) ? (int)$dir['total_dirs'] : 0));
